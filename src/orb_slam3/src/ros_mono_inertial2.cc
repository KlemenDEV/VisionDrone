    ...
    while (1) {
        ...
        if (!img0_buf.empty() && imu_buf.size() > 15) {
            ...
            if (!imu_buf.empty()) {
                ...
                vImuMeas.clear();
                while (imu_buf.front()->header.stamp.toSec() <= time &&
                       imu_buf.front()->header.stamp.toSec() > pretime) {
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "Mono_Inertial2");
    ros::NodeHandle n("~");
    ...
    ros::Subscriber sub_imu = n.subscribe("/imu", 250, imu_callback, ros::TransportHints().tcpNoDelay());
    ros::Subscriber sub_img0 = n.subscribe("/camera/image_raw", 10, img0_callback);
