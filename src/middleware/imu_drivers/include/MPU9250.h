...
class MPU9250 {
    static constexpr uint8_t
    AK8963_WHOAMI_DEFAULT_VALUE { 0x48 };

    // settings
    MPU9250Setting setting;
    // TODO: this should be configured!!
    static constexpr uint8_t
    MAG_MODE { 0x06 };  // 0x02 for 8 Hz, 0x06 for 100 Hz continuous magnetometer data read
...
public:
    ...
    CALIB_GYRO_SENSITIVITY { 131 };     // LSB/degrees/sec
    static constexpr uint16_t
    CALIB_ACCEL_SENSITIVITY { 16384 };  // LSB/g

    bool
    setup(struct I2cDevice *_dev, struct I2cDevice *_dev_mag, const MPU9250Setting &mpu_setting = MPU9250Setting()) {
        setting = mpu_setting;
        dev = _dev;
        dev_mag = _dev_mag;
    ...
    bool available() {
        return has_connected && (read_byte(dev, INT_STATUS) & 0x01);
    }
...
private:
    void initMPU9250() {
        ...
        // reset device
        write_byte(dev, PWR_MGMT_1, 0x80);  // Write a one to bit 7 reset bit; toggle reset device
        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        // wake up device
        write_byte(dev, PWR_MGMT_1, 0x00);  // Clear sleep mode bit (6), enable all sensors
        std::this_thread::sleep_for(std::chrono::milliseconds(100));                                  // Wait for all registers to reset

        // get stable time source
        write_byte(dev, PWR_MGMT_1,
        ...
        uint8_t
                mpu_config = (uint8_t)
        setting.gyro_dlpf_cfg;
        write_byte(dev, MPU_CONFIG, mpu_config);

        // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
        uint8_t
                sample_rate = (uint8_t)
        setting.fifo_sample_rate;
        write_byte(dev, SMPLRT_DIV,
                   sample_rate);  // Use a 200 Hz rate; a rate consistent with the filter update rate
        // determined inset in CONFIG above

        // Set gyroscope full scale range
        // Range selects FS_SEL and GFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
        uint8_t c = read_byte(dev, GYRO_CONFIG);  // get current GYRO_CONFIG register value
        ...
        write_byte(dev, GYRO_CONFIG, c);          // Write new GYRO_CONFIG value to register

        // Set accelerometer full-scale range configuration
        c = read_byte(dev, ACCEL_CONFIG);     // get current ACCEL_CONFIG register value
        c = c & ~0xE0;                                 // Clear self-test bits [7:5]
        c = c & ~0x18;                                 // Clear ACCEL_FS_SEL bits [4:3]
        c = c | (uint8_t(setting.accel_fs_sel) << 3);  // Set full scale range for the accelerometer
        write_byte(dev, ACCEL_CONFIG, c);     // Write new ACCEL_CONFIG register value

        // Set accelerometer sample rate configuration
        // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
        // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
        c = read_byte(dev, ACCEL_CONFIG2);        // get current ACCEL_CONFIG2 register value
        ...
        write_byte(dev, ACCEL_CONFIG2, c);        // Write new ACCEL_CONFIG2 register value
        ...
        write_byte(dev, INT_PIN_CFG, 0x22);
        write_byte(dev, INT_ENABLE, 0x01);  // Enable data ready (bit 0) interrupt
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    ...
    void initAK8963() {
        ...
        write_byte(dev_mag, AK8963_CNTL, 0x00);  // Power down magnetometer
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        write_byte(dev_mag, AK8963_CNTL, 0x0F);  // Enter Fuse ROM access mode
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        read_bytes(dev_mag, AK8963_ASAX, 3, &raw_data[0]);      // Read the x-, y-, and z-axis calibration values
        mag_bias_factory[0] =
                (float) (raw_data[0] - 128) / 256. + 1.;  // Return x-axis sensitivity adjustment values, etc.
        ...
        write_byte(dev_mag, AK8963_CNTL, (uint8_t)
        setting.mag_output_bits << 4 | MAG_MODE);  // Set magnetometer data resolution and sample ODR
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        ...
        if (b_verbose) {
            ...
            printf("Z-Axis sensitivity offset value: %f \n", mag_bias_factory[2]);
        }
    }
    ...
    void update_accel_gyro() {
        ...
        g[0] = (float) raw_acc_gyro_data[4] *
               gyro_resolution;  // get actual gyro value, this depends on scale being set
    ...
    void read_accel_gyro(int16_t *destination) {
        ...
        read_bytes(dev, ACCEL_XOUT_H, 14,
                   &raw_data[0]);             // Read the 14 raw data registers into data array
        destination[0] = ((int16_t) raw_data[0] << 8) |
    ...
    void update_mag() {
        ...
        if (read_mag(mag_count)) {
            ...
            m[0] = (float) (mag_count[0] * mag_resolution * mag_bias_factory[0] - mag_bias[0] * bias_to_current_bits) *
                   mag_scale[0];  // get actual magnetometer value, this depends on scale being set
            m[1] = (float) (mag_count[1] * mag_resolution * mag_bias_factory[1] - mag_bias[1] * bias_to_current_bits) *
                   mag_scale[1];
    ...
    bool read_mag(int16_t *destination) {
        const uint8_t st1 = read_byte(dev_mag, AK8963_ST1);
        if (st1 &
            ...
            read_bytes(dev_mag, AK8963_XOUT_L, 7,
            ...
            if (!(c &
                  0x08)) {                                               // Check if magnetic sensor overflow set, if not then report data
                destination[0] =
                        ((int16_t) raw_data[1] << 8) | raw_data[0];  // Turn the MSB and LSB into a signed 16-bit value
    ...
    int16_t read_temperature_data() {
        ...
        read_bytes(dev, TEMP_OUT_H, 2,
                   &raw_data[0]);  // Read the two raw data registers sequentially into data array
    ...
    void set_acc_gyro_to_calibration() {
        ...
        write_byte(dev, PWR_MGMT_1, 0x01);
        write_byte(dev, PWR_MGMT_2, 0x00);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));

        // Configure device for bias calculation
        write_byte(dev, INT_ENABLE, 0x00);    // Disable all interrupts
        write_byte(dev, FIFO_EN, 0x00);       // Disable FIFO
        write_byte(dev, PWR_MGMT_1, 0x00);    // Turn on internal clock source
        write_byte(dev, I2C_MST_CTRL, 0x00);  // Disable I2C master
        write_byte(dev, USER_CTRL, 0x00);     // Disable FIFO and I2C master modes
        write_byte(dev, USER_CTRL, 0x0C);     // Reset FIFO and DMP
        std::this_thread::sleep_for(std::chrono::milliseconds(15));

        // Configure MPU6050 gyro and accelerometer for bias calculation
        write_byte(dev, MPU_CONFIG, 0x01);    // Set low-pass filter to 188 Hz
        write_byte(dev, SMPLRT_DIV, 0x00);    // Set sample rate to 1 kHz
        write_byte(dev, GYRO_CONFIG,
                   0x00);   // Set gyro full-scale to 250 degrees per second, maximum sensitivity
        write_byte(dev, ACCEL_CONFIG, 0x00);  // Set accelerometer full-scale to 2 g, maximum sensitivity

        // Configure FIFO to capture accelerometer and gyro data for bias calculation
        write_byte(dev, USER_CTRL, 0x40);  // Enable FIFO
        write_byte(dev, FIFO_EN,
    ...
    void collect_acc_gyro_data_to(float *a_bias, float *g_bias) {
        ...
        write_byte(dev, FIFO_EN, 0x00);             // Disable gyro and accelerometer sensors for FIFO
        read_bytes(dev, FIFO_COUNTH, 2, &data[0]);  // read FIFO sample count
        uint16_t
                fifo_count = ((uint16_t)
        data[0] << 8) | data[1];
        ...
        for (uint16_t ii = 0; ii < packet_count; ii++) {
            ...
            read_bytes(dev, FIFO_R_W, 12, &data[0]);              // read data for averaging
    ...
    void write_accel_offset() {
        ...
        int16_t acc_bias_reg[3] = {0, 0,
                                   0};                      // A place to hold the factory accelerometer trim biases
        read_bytes(dev, XA_OFFSET_H, 2, &read_data[0]);  // Read factory accelerometer trim values
        acc_bias_reg[0] = ((int16_t) read_data[0] << 8) | read_data[1];
        read_bytes(dev, YA_OFFSET_H, 2, &read_data[0]);
        acc_bias_reg[1] = ((int16_t) read_data[0] << 8) | read_data[1];
        read_bytes(dev, ZA_OFFSET_H, 2, &read_data[0]);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            acc_bias_reg[i] -=
                    (int16_t) acc_bias[i] >> 3;  // Subtract calculated averaged accelerometer bias scaled to 2048 LSB/g
            if (mask_bit[i]) {
        ...
        write_byte(dev, XA_OFFSET_H, write_data[0]);
        write_byte(dev, XA_OFFSET_L, write_data[1]);
        write_byte(dev, YA_OFFSET_H, write_data[2]);
        write_byte(dev, YA_OFFSET_L, write_data[3]);
        write_byte(dev, ZA_OFFSET_H, write_data[4]);
        write_byte(dev, ZA_OFFSET_L, write_data[5]);
    }
    ...
    void write_gyro_offset() {
        ...
        write_byte(dev, XG_OFFSET_H, gyro_offset_data[0]);
        write_byte(dev, XG_OFFSET_L, gyro_offset_data[1]);
        write_byte(dev, YG_OFFSET_H, gyro_offset_data[2]);
        write_byte(dev, YG_OFFSET_L, gyro_offset_data[3]);
        write_byte(dev, ZG_OFFSET_H, gyro_offset_data[4]);
        write_byte(dev, ZG_OFFSET_L, gyro_offset_data[5]);
    }
    ...
    void calibrate_mag_impl() {
        ...
        if (b_verbose) {
            ...
            printf("%f", mag_scale[0]);
            printf(", ");
            printf("%f", mag_scale[1]);
            printf(", ");
            printf("%f", mag_scale[2]);
            printf("\n");
        }
    ...
    bool
    self_test_impl()  // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        write_byte(dev, SMPLRT_DIV, 0x00);       // Set gyro sample rate to 1 kHz
        write_byte(dev, MPU_CONFIG, 0x02);       // Set gyro sample rate to 1 kHz and DLPF to 92 Hz
        write_byte(dev, GYRO_CONFIG, FS << 3);   // Set full scale range for the gyro to 250 dps
        write_byte(dev, ACCEL_CONFIG2, 0x02);    // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
        write_byte(dev, ACCEL_CONFIG, FS << 3);  // Set full scale range for the accelerometer to 2 g

        for (int ii = 0; ii < 200; ii++) {  // get average current values of gyro and acclerometer

            read_bytes(dev, ACCEL_XOUT_H, 6,
                       &raw_data[0]);          // Read the six raw data registers into data array
            aAvg[0] += (int16_t)(
                    ((int16_t) raw_data[0] << 8) | raw_data[1]);  // Turn the MSB and LSB into a signed 16-bit value
            aAvg[1] += (int16_t)(((int16_t) raw_data[2] << 8) | raw_data[3]);
            aAvg[2] += (int16_t)(((int16_t) raw_data[4] << 8) | raw_data[5]);

            read_bytes(dev, GYRO_XOUT_H, 6,
                       &raw_data[0]);           // Read the six raw data registers sequentially into data array
            gAvg[0] += (int16_t)(
                    ((int16_t) raw_data[0] << 8) | raw_data[1]);  // Turn the MSB and LSB into a signed 16-bit value
        ...
        write_byte(dev, ACCEL_CONFIG, 0x00);
        write_byte(dev, GYRO_CONFIG, 0x00);
        std::this_thread::sleep_for(std::chrono::milliseconds(25));  // Delay a while to let the device stabilize

        // Retrieve accelerometer and gyro factory Self-Test Code from USR_Reg
        uint8_t self_test_data[6];
        self_test_data[0] = read_byte(dev, SELF_TEST_X_ACCEL);  // X-axis accel self-test results
        self_test_data[1] = read_byte(dev, SELF_TEST_Y_ACCEL);  // Y-axis accel self-test results
        self_test_data[2] = read_byte(dev, SELF_TEST_Z_ACCEL);  // Z-axis accel self-test results
        self_test_data[3] = read_byte(dev, SELF_TEST_X_GYRO);   // X-axis gyro self-test results
        self_test_data[4] = read_byte(dev, SELF_TEST_Y_GYRO);   // Y-axis gyro self-test results
        self_test_data[5] = read_byte(dev, SELF_TEST_Z_GYRO);   // Z-axis gyro self-test results
        ...
        for (int i = 0; i < 3; i++) {
            self_test_result[i] = 100.0 * ((float) (aSTAvg[i] - aAvg[i])) / factoryTrim[i] -
    ...
    void write_byte(struct I2cDevice *i2cdev, uint8_t subAddress, uint8_t data) {
        uint8_t rega[] = {subAddress, data};
        i2c_write(i2cdev, rega, 2);
    }

    uint8_t read_byte(struct I2cDevice *i2cdev, uint8_t subAddress) {
        uint8_t rega[] = {subAddress};
        i2c_write(i2cdev, rega, 1);

        uint8_t inbuf[1];
        i2c_read(i2cdev, inbuf, 1);

        return inbuf[0];
    }

    void read_bytes(struct I2cDevice *i2cdev, uint8_t subAddress, uint8_t count, uint8_t *dest) {
        uint8_t rega[] = {subAddress};
        i2c_write(i2cdev, rega, 1);

        uint8_t inbuf[count];
        i2c_read(i2cdev, inbuf, count);
        memcpy(dest, inbuf, count);
    }
