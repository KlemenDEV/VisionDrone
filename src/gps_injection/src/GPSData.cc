...
void GPSData::consume(const ublox_msgs::NavPVT7::ConstPtr &msg) {
    ...
    this->ned_north = (int32_t) (msg->velN / 10); // NED north velocity [mm/s -> cm/s]
    this->ned_east = (int32_t) (msg->velE / 10); // NED east velocity [mm/s -> cm/s]
    this->ned_down = (int32_t) (msg->velD / 10); // NED down velocity [mm/s -> cm/s]
    this->speed_2d = (uint32_t) (msg->gSpeed / 10); // Ground Speed (2-D) [mm/s -> cm/s]
    this->heading_2d = msg->heading; // Heading of motion 2-D [deg / 1e-5]
    this->fix_type = msg->fixType;
